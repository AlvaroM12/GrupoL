CREATE TABLE USUARIO (ID BIGINT NOT NULL, DTYPE VARCHAR(31), TELEFONO BIGINT, CONTRASEÑA VARCHAR, PRIMARY KEY (ID))
CREATE TABLE GRUPO (ID BIGINT NOT NULL, ASIGNAR BIGINT, CURSO BIGINT UNIQUE, INGLES VARCHAR, LETRA VARCHAR UNIQUE, PLAZAS BIGINT, TURNO_MAÑANA_TARDE VARCHAR, VISIBLE BIGINT, GG_ID BIGINT, TG_CÓDIGO BIGINT, PRIMARY KEY (ID))
CREATE TABLE ALUMNO (ID BIGINT NOT NULL, APELLIDO1 VARCHAR, APELLIDO2 VARCHAR, CP BIGINT, DNI VARCHAR UNIQUE, DIRECCION VARCHAR, EMAIL_INSTITUCIONAL VARCHAR, EMAIL_PERSONAL VARCHAR, LOCALIDAD VARCHAR, MÓVIL BIGINT, NOMBRE VARCHAR, PROVINCIA VARCHAR, PRIMARY KEY (ID))
CREATE TABLE ASIGNATURA (REFERENCIA BIGINT NOT NULL, DTYPE VARCHAR(31), CODIGO BIGINT, CRÉDITOS_PRACTICOS BIGINT, CRÉDITOS_TEORICOS BIGINT, CUATRIMESTRE VARCHAR, CURSO BIGINT, IDIOMA_DE_IMPARTICION VARCHAR, NOMBRE VARCHAR, OFERTADA VARCHAR, PLAZAS VARCHAR, TOTAL_CRÉDITOS BIGINT, TA_CÓDIGO BIGINT, PRIMARY KEY (REFERENCIA))
CREATE TABLE ASIGNATURAS_MATRÍCULA (G_AM_ID BIGINT, ASIGNATURA_REFERENCIA BIGINT NOT NULL, Curso_Academico VARCHAR NOT NULL, EM BIGINT NOT NULL, PRIMARY KEY (ASIGNATURA_REFERENCIA, Curso_Academico, EM))
...
CREATE TABLE EXPEDIENTE (NUM_EXPEDIENTE BIGINT NOT NULL, ACTIVO VARCHAR, CREDITOS_CF DOUBLE, CREDITOS_FB DOUBLE, CREDITOS_OB DOUBLE, CREDITOS_OP DOUBLE, CREDITOS_PE DOUBLE, CREDITOS_SUPERADOS DOUBLE, CREDITOS_TF DOUBLE, NOTA_MEDIA DOUBLE, AE_ID BIGINT, TE_CÓDIGO BIGINT, PRIMARY KEY (NUM_EXPEDIENTE))
...
CREATE TABLE MATRÍCULA (CURSO_ACADEMICO VARCHAR NOT NULL, ESTADO VARCHAR, FECHA_DE_MATRÍCULA DATE, NUEVO_INGRESO VARCHAR, NUM_ARCHIVO BIGINT, TURNO_PREFERENTE VARCHAR, EM BIGINT NOT NULL, PRIMARY KEY (CURSO_ACADEMICO, EM))
CREATE TABLE OPTATIVA (REFERENCIA BIGINT NOT NULL, MENCIÓN VARCHAR, PRIMARY KEY (REFERENCIA))
CREATE TABLE PERSONAL_DE_SECRETARIA (ID BIGINT NOT NULL, RANGO VARCHAR, PRIMARY KEY (ID))
CREATE TABLE TITULACION (CÓDIGO BIGINT NOT NULL, CRÉDITOS BIGINT, NOMBRE VARCHAR, PRIMARY KEY (CÓDIGO))
...
CREATE TABLE MATRÍCULA_ASIGNATURA (CURSO_ACADEMICO VARCHAR NOT NULL, EM BIGINT NOT NULL, a_REFERENCIA BIGINT NOT NULL, PRIMARY KEY (CURSO_ACADEMICO, EM, a_REFERENCIA))
...
ALTER TABLE ASIGNATURAS_MATRÍCULA ADD CONSTRAINT FK_ASIGNATURAS_MATRÍCULA_Curso_Academico FOREIGN KEY (Curso_Academico, EM) REFERENCES MATRÍCULA (CURSO_ACADEMICO, EM)
...
ALTER TABLE MATRÍCULA ADD CONSTRAINT FK_MATRÍCULA_EM FOREIGN KEY (EM) REFERENCES EXPEDIENTE (NUM_EXPEDIENTE)
...
ALTER TABLE MATRÍCULA_ASIGNATURA ADD CONSTRAINT FK_MATRÍCULA_ASIGNATURA_CURSO_ACADEMICO FOREIGN KEY (CURSO_ACADEMICO, EM) REFERENCES MATRÍCULA (CURSO_ACADEMICO, EM)
ALTER TABLE MATRÍCULA_ASIGNATURA ADD CONSTRAINT FK_MATRÍCULA_ASIGNATURA_a_REFERENCIA FOREIGN KEY (a_REFERENCIA) REFERENCES ASIGNATURA (REFERENCIA)
